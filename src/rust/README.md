...
```toml
[dependencies]
dtlvnative = { version = "1.1.3", features = ["usearch", "llama"] }
```

For storage alone, use `dtlvnative = "1.1.3"`.

The crates use prebuilt native libraries downloaded from
[GitHub releases](https://github.com/datalevin/dtlvnative/releases/tag/1.1.3).
...
```toml
[dependencies]
dtlvnative = { version = "1.1.3", default-features = false, features = ["usearch"] }
```
